...
class UserFeat(nn.Module):
    def __init__(self, map_vocab=None):
        super().__init__()
        ...
        for attr in self.attr_ls:
            setattr(self, f'{attr}_emb', nn.Embedding(num_embeddings=getattr(const, f'{attr}_num'), embedding_dim=getattr(const, f'{attr}_dim')))
...
class ItemFeat(nn.Module):
    def __init__(self, query_feat, map_vocab=None):
        ...
        for attr in self.attr_ls:
            if attr in const.item_text_feature:
                ...
                setattr(self, f'{attr}_emb', nn.Embedding(num_embeddings=getattr(const, f'{attr}_num'), embedding_dim=getattr(const, f'{attr}_dim'), padding_idx=0))
                nn.init.xavier_normal_(getattr(self, f'{attr}_emb').weight.data)
    ...
    def forward(self, sample):
        ...
        result_emb = torch.zeros((new_sample.shape[0], const.final_emb_size), device=sample.device)
        ...
        if sub_mask.sum() > 0:
            ...
            for attr in self.attr_ls:
                ...
                feats_ls.append(getattr(self, f'{attr}_emb')(index))
            sub_sample_emb = torch.tanh(self.item_trans(torch.cat(feats_ls, dim=-1))).clone()
...
class QueryEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.word_embedding = nn.Embedding(num_embeddings=const.word_id_num, embedding_dim=const.word_id_dim, padding_idx=0)
...
class QueryFeat(nn.Module):
    ...
    def forward(self, sample):
        query_emb = self.query_encoder(sample.reshape((-1, const.max_query_word_len)))
...
class SrcSessionFeat(nn.Module):
    ...
    def forward(self, sample):
        ...
        result_query_emb = torch.zeros((new_sample.shape[0], const.final_emb_size), device=sample.device)
        result_item_emb = torch.zeros((new_sample.shape[0], const.max_session_item_len, const.final_emb_size), device=sample.device)
        result_item_mask = torch.zeros((new_sample.shape[0], const.max_session_item_len), device=sample.device).bool()
        ...
        if sub_mask.sum() > 0:
            ...
            sub_click_item_mask = torch.where(sub_click_item_ls == 0, 0, 1).bool()
        ...
        result_query_emb = result_query_emb.reshape((*sample.shape, const.final_emb_size))
        result_item_emb = result_item_emb.reshape((*sample.shape, const.max_session_item_len, const.final_emb_size))
        result_item_mask = result_item_mask.reshape((*sample.shape, const.max_session_item_len))
